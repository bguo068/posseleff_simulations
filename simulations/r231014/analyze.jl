...
const RESDIR::String = "/local/chib/toconnor_grp/bing/posseleff_simulations/simulations/r231014/resdir/"
...
function plot_totibd(f::Figure, axes::Dict{NTuple{4,Int},Axis})
    for sim in SIMS
        ...
        ax = get_ax(axes, sim)
        try
            ibd = read_ibd_obj_ibd_only(get_ibd_obj_fname(RESDIR, sim.gsid, sim.label))
            m = get_total_mat(ibd)
            o = clust_total_mat(m)
            m2 = m[o, o]
