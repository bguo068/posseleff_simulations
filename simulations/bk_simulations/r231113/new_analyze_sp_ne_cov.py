...
RESDIR: str = "/local/scratch/bing/posseleff_simulations/simulations/r231113/resdir"
...
def get_ibd_obj_ifm_rmpeaks_path(resdir, label, gsid):
    # "resdir/10000_sp_neu_rep0/ibddist_ibd/10000_2.0_10.0_none.ibddist.ibdobj.gz"
    # TODO: change it back to rmpeaks from orig (once need data is generated)
    p = f"{resdir}/{gsid}_{label}/ibdne_ibd/{gsid}_2.0_10.0_none_orig.ibdne.ibdobj.gz"
    assert Path(p).exists(), f"{p} does not exist"
    return p


def get_ibd_obj_ifm_ibdne_path_repo(resdir, label, gsid, repno):
    gsid = gsid + 10 * repno
    p = f"{resdir}/{gsid}_{label}_rep{repno}/ibdne_ibd/{gsid}_2.0_10.0_none_orig.ibdne.ibdobj.gz"
    assert Path(p).exists(), f"{p} does not exist"
    return p
...
def get_restart_count(resdir, label, gsid, repno=None):
...
for label in SIMULATIONS.keys():
    ...
    p = get_ibd_obj_ifm_ibdne_path_repo(RESDIR, label, gsid, repno=REPNO)
...
fig.savefig("new_coverage_sp.png")
...
for label in SIMULATIONS.keys():
    ...
    if i == 0:
        ax.set_title(get_title(j))
    df_true = read_true_ne(get_true_ne_path_rep(RESDIR, label, gsid, repno=REPNO))
    df_orig = read_ne(
        get_ne_path_rep(
            RESDIR,
            label,
