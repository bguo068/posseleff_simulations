...
class SlimMsprimeSimulatorForSinglePop:
    ...
    def simulate_genome(
        ...
        # make a genome-wide rate map
        # ====== example from msprime manual
        #
        #       rates = []
        ...
        # =======
        r_chrom = self.params.r
        r_break = math.log(2)
        positions = [0]
        rates = []
        for i in range(1, self.params.nchrs + 1):
            positions += [self.params.seqlen * i]
            rates += [r_chrom]
            if i != self.params.nchrs:
                positions += [self.params.seqlen * i + 1]
                rates += [r_break]
        ...
        logging.info("run msprime mutate")
        # mutate
...
if __name__ == "__main__":
    ...
    for ichr in range(simulator.params.nchrs):
        ...
        L = simulator.params.seqlen
        nchrs = simulator.params.nchrs
        intervals = [[ichr * L, (ichr + 1) * L - 1]]

        logging.info(f"run cut mutated trees by interval: chr{chrno}")
        # mutated trees
        mts = simulator.mutated_trees.keep_intervals(intervals).trim()
        # write vcf
        ...
        simulator.simplified_trees.keep_intervals(intervals).trim().dump(ofn_tree)
